...
import { sampleWithRequiredData } from '../aircraft.test-samples';
...
describe('Aircraft Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 14531 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.aircrafts()[0]).toEqual(expect.objectContaining({ id: 14531 }));
  });

  describe('trackId', () => {
    it('should forward to aircraftService', () => {
      const entity = { id: 14531 };
      vitest.spyOn(service, 'getAircraftIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAircraftIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.aircraftsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
