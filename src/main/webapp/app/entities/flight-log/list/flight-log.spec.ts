...
import { sampleWithRequiredData } from '../flight-log.test-samples';
...
describe('FlightLog Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: FlightLog;
  let fixture: ComponentFixture<FlightLog>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(FlightLog);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 15441 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.flightLogs()[0]).toEqual(expect.objectContaining({ id: 15441 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 15441 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.flightLogs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 15441 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to flightLogService', () => {
      const entity = { id: 15441 };
      vitest.spyOn(service, 'getFlightLogIdentifier');
      const id = comp.trackId(entity);
      expect(service.getFlightLogIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.flightLogsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
