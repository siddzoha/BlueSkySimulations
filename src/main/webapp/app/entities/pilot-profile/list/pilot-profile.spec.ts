...
import { sampleWithRequiredData } from '../pilot-profile.test-samples';
...
describe('PilotProfile Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PilotProfile;
  let fixture: ComponentFixture<PilotProfile>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PilotProfile);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 18599 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.pilotProfiles()[0]).toEqual(expect.objectContaining({ id: 18599 }));
  });

  describe('trackId', () => {
    it('should forward to pilotProfileService', () => {
      const entity = { id: 18599 };
      vitest.spyOn(service, 'getPilotProfileIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPilotProfileIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.pilotProfilesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
