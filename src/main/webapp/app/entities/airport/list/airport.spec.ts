...
import { sampleWithRequiredData } from '../airport.test-samples';
...
describe('Airport Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 18443 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.airports()[0]).toEqual(expect.objectContaining({ id: 18443 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 18443 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.airports()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 18443 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to airportService', () => {
      ...
      vitest.spyOn(service, 'getAirportIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAirportIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.airportsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
