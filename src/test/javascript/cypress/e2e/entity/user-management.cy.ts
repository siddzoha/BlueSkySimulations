...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Andres22', email: 'Gunnar90@hotmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Violet_Muller18');
      cy.get(`[data-cy="login"]`).should('have.value', 'Violet_Muller18');

      cy.get(`[data-cy="firstName"]`).type('Jaida');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Jaida');

      cy.get(`[data-cy="lastName"]`).type('Littel');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Littel');

      cy.get(`[data-cy="email"]`).type('Diego_Satterfield@yahoo.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Diego_Satterfield@yahoo.com');
